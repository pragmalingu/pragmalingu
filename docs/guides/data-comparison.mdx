...
```
**Queries:**<br /> There are over one million (1,010,916) unique real queries which are assigned to an unique ID. The queries can all be found in the Q&A corpus, for the Documents Ranking corpus there were only 1/8 of the queries used. Those were split into train, development and test set. We used the development set for our experiments because for using the Ranking Evaluation API of Elasticsearch you need relevance assessments. Those were only provided by the development set.
The development queries (5,193) are stored in the file `msmarco-docdev-queries.tsv`. Every row of the file holds the query and it's ID separated by one tab. 
...
**Other files:** <br />
`msmarco-docs-lookup.tsv` <br />
`msmarco-doctrain-queries.tsv` <br />
`msmarco-docdev-top100` <br />
`docleaderboard-queries.tsv`<br />
`docleaderboard-top100` <br />
