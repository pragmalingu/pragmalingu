...
If you don't want to determine the markers on your own, we have already done all the work by analyzing every single [Data Set](../guides/data-comparison.mdx) and even wrote an [overview table](../comparisons/overview.mdx).
...
Since it's easier to demonstrate parsing by using a practical example, we picked one of our [8 freely available Data Sets](../comparisons/data-comparison.mdx).
