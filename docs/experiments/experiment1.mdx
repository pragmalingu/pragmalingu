...
For our first experiment we connected a Google Colab Notebook to our Elasticsearch instance and compared the standard Elasticsearch operator with two built-in search metrics.
...
At first we evaluated the standard analyzer using a [multi match query](https://www.elastic.co/guide/en/elasticsearch/reference/current/query-dsl-simple-query-string-query.html) which searches through the field 'text' and 'title'. We only looked at the first 20 retrieved documents and measured precision and recall. 
...
These are the results without any further processing of the data using the default analyzer of Elasticsearch:
...
Elasticsearch provides two built-in stemming methods. 
First of, is the algorithmic stemming, it applies a series of rules to each word to reduce them to their base form or stem. In Elasticsearch this can be achieved with the [stemmer token filter](https://www.elastic.co/guide/en/elasticsearch/reference/current/analysis-stemmer-tokenfilter.html). 

Second of, the dictionary stemming, which replaces unstemmed words with stemmed variants from a provided dictionary. The Elasticsearch built-in counterpart is called [hunspell token filter](https://www.elastic.co/guide/en/elasticsearch/reference/current/analysis-hunspell-tokenfilter.html). 
...
It's the better choice in most cases because it needs less memory and is considered faster than a dictionary approach. It's usage in Elasticsearch is also not complicated and can be configured quickly.
But irregular words or names could result in strange forms that don't give any helpful information since it is only rule-based.

Yet we expected an improvement over the standard Elasticsearch analyzer.

This is how we configured the stemming analyzer:
...
Afterwards we added it to the "settings" of our indexes where we stored the documents of each corpus:
...
Surprisingly, the improvements over the standard analyzer are marginal.
