...
For our second experiment we connect a Colab Notebook to an Elasticsearch instance and compared the standard Elasticsearch operator with our first embedding approach. Therefore, we transformed the documents into vector representations by using a BERT model and indexed them. Afterwards we searched in the transformed documents with a vector representation of the query.
Since the BERT model is implemented to work on sentences, we compared a search just on the document titles as well as on the whole text of the documents.

We already described how the standard Elasticsearch operator works and how we implemented it in [our first experiment](./experiment1.mdx).

<details>
<summary>Need a short summary of the Multi-Match Query?</summary>  

The standard Elasticsearch method - [Multi-Match Query](https://www.elastic.co/guide/en/elasticsearch/reference/current/query-dsl-simple-query-string-query.html) - is a full-text search that comes with standard tokenization and an analyzer. It searches only the given fields of the documents - in our case 'text' and 'title'. To gather our data, we looked only at the first 20 retrieved documents for any given search.

</details>  

### 1. What Is BERT?

BERT is short for Bi-Directional Encoder Representation from Transformer and is an approach of using large pre-trained neural networks with some exceptional solutions to get vector representations - embeddings - from texts. With those embeddings we can measure some similarity metrics like cosine similarity to compare the meaning of these texts.
...
For initializing the model we run this code:
...
After initializing the BERT model, we converted the documents and the search queries into sentence embeddings. Then - same as in our last experiment - we calculated comparable values using the [Ranking Evaluation API](../guides/ranking-api.mdx).

#### 3.1. Preparing The Documents
Now that the model was integrated in our Notebook, we could start transforming our texts with BERT. To be able to compare the documents and the search query using cosine similarity, we first had to convert the desired search fields into embeddings and feed them into Elasticsearch.
Since we worked with the Elasticsearch param `dense_vector`, it is not possible to recreate this experiment on Open Distro for Elasticsearch, you have to use the standard Elasticsearch installation instead.

**Title Embeddings**<br></br>
As a first approach, we tried to calculate the similarity between the search queries and the titles of the documents. For this purpose, we set `title_vector` as a container for the computed BERT embedding in addition to the field `title`, which should be the container for the text of the title:
...
The procedure for the whole text was a bit more complicated. We first had to split the text into sentences, otherwise the embeddings would have been far too long or  even cut of some of the text. Therefore, we created a new category inside the corpus dictionaries to store the split sentences for easy access:
...
Since a text is represented by more than one sentence, we needed to create a container to store all the sentence embeddings. Therefore we initialized `text_vector` as seen below, which is a nested container. The the whole unsplit text is stored in the container `text`:
...
Now that we prepared all the documents, we only had to compute the search queries before we could pass them to the Ranking Evaluation API. For the search on the title fields a `script_score` request for each query was enough to get some results. Our `script_score` calculates the cosine similarity between each document title and the search query before returning the top 20 results:
...
In order to get suitable comparison values, we decided to carry out the multi-match query in the same way as the BERT search on the title or text field. Since the data sets `Medline`,` NPL` and `Time` do not have any titles, the title search was left out for these corpora.
Same procedure as in the last experiment: To compare our results properly we measured Recall, Precision and F1-Score for every method on every corpus.
...
<a class="buttons" href=""><button class="buttons" >Try it yourself!</button></a>