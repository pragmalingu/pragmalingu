...
**Description:**<br /> The LISA collection, lend in 1982 by Peter Willett of Sheffield University, is being provided to support research investigations. 
...
**Parsing problems**:<br />
* documents: they are all in separate files; having the newline as inline marker to separate ID, title and text has proven to be a bit tricky;
...
The end marker structure is acutely consistent for every file, even though it lacks context to engage with in a pragmatic search.
...
* `term-mst` contains word ID, followed by context-word ID, followed by co.occurrences and mutual similarity values. This is only available for words that occur in at least 2 documents
...
**Description:**<br />Uses same notation as `CACM` and `CISI`, but the relevance evaluations are more detailed and more specific to the query tasks. To use this collection it's important to note that the ID of the queries `.I 002` is not the same query-ID used in the relevance assessments. It seems that the query-IDs come from the order in the file. It could be helpful to update the IDs before working with it to avoid confusion.
...
**Description:**<br /> This collection was originally amassed and labeled by Carnegie Group, Inc. and Reuters, Ltd. Further informations is available in the `README.txt` document. <br />
