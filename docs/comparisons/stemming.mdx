...
The [Porter stemming algorithm](https://snowballstem.org/algorithms/porter/stemmer.html) created by [Martin Porter](https://en.wikipedia.org/wiki/Martin_Porter) is one of the most popular algorithmic stemmers 
...
Algorithmic stemmers are very fast and don't need a lot of memory because they only apply a set of rules, that represent the common rules how words are [inflected](https://en.wikipedia.org/wiki/Inflection) in a specific language.  
Because there are always exceptions to a rule, an algorithmic stemmer might produce the same stem for words that have different meanings.
...
Dictionary stemmers should work better on irregular words or words that are similar but have different meanings.
...
Dictionary stemmers also have the draw back that they are slower and require more memory because they have to look up all words in their dictionary.
...
## 2. Results
The following are the results from our [first experiment](../experiments/experiment1). You can reproduce these results yourself if you follow the instructions on our [experiment section](../experiments/experiments-intro).
...
Looking at the f1-score from our approaches we can see that both stemming approaches perform better than the standard baseline on most datasets.
...
Hunspell stemming performs worse than algorithmic stemming on most datasets except NPL where it performs slightly better.
Hunspell stemming performs worse than the standard analyzer on the medline dataset.

There are a list of interesting questions that we can ask by just looking at these results:
...
4.   Why is Hunspell stemming performing better than the algorithmic stemmer on the NPL dataset?
...
We see from the gains figure that algorithmic stemming performs better than the standard analyzer on all tested datasets.
...
The overall document score with the Hunspell stemmer of **16.61** is the sum of these individual word scores:
...
We are looking at the CACM and ADI dataset which show the most significant differences to find examples that explain this performance.

First we look at the CACM dataset to find good examples.  
We calculate the f-score differences for both approaches with our search analysis tool and pick query **26** because it exhibits the biggest f-score difference between the two approaches.  
...
This happens because we are using the default scoring strategy for the multi match query that is [**best_field**](https://www.elastic.co/guide/en/elasticsearch/reference/current/query-dsl-multi-match-query.html#multi-match-types), 
...
### Standard analyzer improvements over Hunspell:
...
favoring the standard analyzer to find examples that explain this performance drawback of the Hunspell stemmer.
...
The overall document score with the standard analyzer was **6.22** and was the sum of the following keyword scores: 
...
As algorithmic stemming is built in to the language analyzers provided by Elasticsearch it is fairly easy to use.
