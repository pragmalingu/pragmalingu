...
We discuss and present the results of our in depth experiments here, so that the usefulnes of these approaches can be assessed quickly.

For our first comparsion we looked at stemming algorithms, a standard approach that is in use in search engines for decades. We used the available stemmer 
token filters in Elasticsearch and compared them to the default search setup without any stemming.
You can read more about what stemming is and how usefull it is for search in our [stemming comparison](http://localhost:3000/docs/comparisons/stemming).
...
natural language processing tasks. You can read more about embeddings and the current state of the art in language modeling in our [guides section](http://pragmalingu.de/docs/guides/wordembeddings-intro). Or you can check out
