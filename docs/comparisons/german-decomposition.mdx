...
Since German has some specific language features which English doesn't provide which - if taken into account - could make a difference in the search result, we decided to to work on german texts for this experiment. In order to keep the same [corpora](../guides/data-comparison) we already used in other experiments the [DeepL API](https://www.deepl.com/pro#developer) to translate all our documents and queries into german. For our first comparison on the german corpus we looked into splitting compound words using decomposition. To understand why decomposition could increase the search results it's necessary to know what composition means.
...
Based on the scores of the **Standard Analyzer**, the outcome became better with the **German Analyzer**. The **Dictionary Decompounder** was able to increase the values once more. On top of that the increase due to the **Hyphentation Decompounder** showed the highest score.
...
|                                |  score       | position |
|--------------------------------|--------------|----------|
| **German Analyzer**            | 3.3543134    |     -    |
| **Hyphenation Decompounder**   | 28.156767    |     1    |


When we tried to explain why the **German Analyzer** works better than the **Hyphenation Decompounder** we have to look at the search results in detail.

**German Analyzer**
"position": outside of k, "score": 3.3543134
```json
{
        "_id" : "81",
        "_score" : 3.3543134,
...
The **German Analyzer** seems to find way less than the **Hyphenation Decompounder**. Even though some of the terms the **Hyphenation** approach extracts are quite unusefull for this query, like `well` and `reich`, it still finds enough relevant terms that weren't found by only using the standard approach.
...
Second we compared the **Dictionary Decompounder** with the **Hyphenation Decompounder** to see how much the improvement between a dictionary and a hyphenation approach weights. We used **Query 42** for this comparison, since the f-score difference is `0.25`.

*Example Query 42*: "LÖSUNG VON DIFFERENTIALGLEICHUNGEN PER COMPUTER"

The **Hyphenation Analyzer** finds way more true positives than the **Dictionary Decompounder**, out of 36 relevant documents only 1 is found by the **Dictionary** approach and 8 by the **Hyphenation** approach.
...
While experimenting the scores for the **German Analyzer** always exceeded the scores of the **Standard Analyzer** and on some, like [CACM](../guides/data-comparison#cacm) or [LISA](../guides/data-comparison#lisa), there was a increase of nearly 50%, even around 80% with the [ADI](../guides/data-comparison#adi) (see graphic with gains in section [3. Results](../comparisons/german-decomposition#3-results)).
