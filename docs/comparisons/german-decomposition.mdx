...
We configured the **Dictionary Decompounder** for the settings of every corpus following the [Elasticsearch documentation](https://www.elastic.co/guide/en/elasticsearch//reference/master/analysis-dict-decomp-tokenfilter.html):
...
We configured the **Hyphenation Decompounder** for the settings of every corpus following the [Elasticsearch documentation](https://www.elastic.co/guide/en/elasticsearch/reference/current/analysis-hyp-decomp-tokenfilter.html):
...
Once again it is helpful to look at one relevant document from the query more closely. The document with the id `5444`, which we chose as a an example, does not get ranked into the top 20 retrieved documents on the **dictionary** approach, but is on the second position with the **hyphenation** approach. 
...
As we can see, the **Dictionary Decompounder** seems to search on more sub terms for `differentialgleichung`than the **Hyphneation Decompounder**. This leads to a higher document frequency which reduces the term score. So the **Dictionary** approach searches terms that aren't relevant for the query and therefore is more likely to score relevant documents lower than irrelevant ones.

**Conclusion**
On the NPL the **Hyphenation Decompounder** seems to work best. It seems to differenciate between necessary decompounding and words that only raise the document frequency. This examples underline our hypothesis that raking decomposition into account can effect the search scores positivly.

### 5.2. Why is the German Analyzer significantly better than the Standard Analyzer?

While experimenting the scores for the **German Analyzer** always exceeded the scores of the **Standard Analyzer** and on some corpora, like [CACM](../guides/data-comparison#cacm) or [LISA](../guides/data-comparison#lisa), there was a increase of nearly 50%, even around 80% with the [ADI](../guides/data-comparison#adi) (see graphic with gains in section [3. Results](../comparisons/german-decomposition#3-results)).
...
To analyze why the results of the **German Analyzer** seem to be better than the decompounding approaches, we chose the **Dictionary Decompounder** for the comparision.
...
We chose the one retrieved document that was returned by both approches, `181`. 
We can see by the scores and positions, that apparently the **DictionaryDecompounder** matches way more documents before matching 181 although the score isn't that different from the **German Analyzer**:
...
Although the decompounding matches way more terms inside the documents `text_german` field, both approaches seem to rely more on the `title_german` field score because it was higher:
...
The **Dictionary Decompounder** finds way more terms, also including some irrelevant and non-sense words like `aktull` or `formation` w(hich has nothing to do with the searched term `information`).
...
This concludes that with all those irrelevant `terms` the **Dictionary Decompounder** seems to have the same problem on the LISA corpus as on the Cranfield, the decomposition increases the returned document rate which decreases the idf and therefore matches more irrelevant before the relevant documents.
...
**Conclusion**
This examples show that sometimes decomposition can lead to a decrease of the values due to over-decompounding the search terms and therefore returning more noise.

### 5.4. Why does the Dictionary Decompounder work better than Hyphenation Decompounder?

While we mostly looked at the comparison between the decompounding and standard approaches, the **Dictionary Decompounder** was always subject to the **Hyphenation Decompounder**. Only in the [Medline](../guides/data-comparison#medline) and the [CACM](../guides/data-comparison#cacm) corpus there were minor deviations in the scores where the **Dictionary Decompounder** did better.
...
To illustrate what exactly caused the increase, we picked out a search query that seemed most informative to us. 
...
As a good example document to compare the scores and searched terms in detail we chose the document `732` because the **Hyphenation Decompounder** doesn't score it into the top 20 decouments, while the **Dictionary Decompounder** does.
